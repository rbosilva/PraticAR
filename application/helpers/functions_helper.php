        ...
        if (!empty($params['search']['value'])) {
            ...
            foreach ($params['columns'] as $column) {
                if ($column['searchable'] === 'true') {
                    // Caso esteja utilizando um alias, passe o nome do campo na
                    // propriedade "name" das "columns" do datatables
                    if (!empty($column['name'])) {
                        $where[] = "lower($column[name]) like lower('%$search_value%')";
                    } else {
                        $where[] = "lower($column[data]) like lower('%$search_value%')";
                    }
                }
            }
...
if (!function_exists('format_results')) {
    ...
    function formatResults(array $data, $recordsTotal, $recordsFiltered, $draw, $callback = null) {
        foreach ($data as &$row) {
            ...
            $row['editar'] = '<a href="#" title="Editar" class="editar"><i class="fa fa-pencil"></i></a>';
            $row['excluir'] = '<a href="#" title="Excluir" class="excluir"><i class="fa fa-remove"></i></a>';
